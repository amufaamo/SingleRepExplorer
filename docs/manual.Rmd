---
title: "SingleRepExplorer Manual (v1.0.0)"
...
### Saving and Restoring Sessions (.qs2)

After running the analysis pipeline, the entire session state—including the processed Seurat object, repertoire annotations, custom Lasso subsets, and all parameter settings—can be exported as a single `.qs2` file via the **"Save Session (.qs2)"** button at the top-right of the navigation bar.

* The exported file can be **re-uploaded** later in the **Data Upload** tab to instantly restore the analysis without re-running the pipeline.
* Saved sessions are also useful for sharing intermediate results with collaborators or for resuming work across machines.
* The `.qs2` format (powered by the `qs2` R package) provides fast read/write performance compared to `.rds`.

Optionally, the underlying processed Seurat object can also be downloaded as a standard `.rds` file for use in custom R workflows outside the application.
...
### 3. Memory-Preserved Quality Control (QC)

A distinguishing feature of SingleRepExplorer is **memory-preserved QC**: the raw Seurat object is retained in memory after the initial upload, so QC thresholds can be **iteratively adjusted without re-uploading data**.

When you upload an `.h5` file, the application immediately generates:

* **Interactive QC scatter plots** (gene count vs. mitochondrial percentage, etc.)
* **Violin plots** of `nFeature_RNA`, `nCount_RNA`, and `percent.mt`
* **Live "Estimated Cells Remaining" counter** that updates in real-time as you change thresholds

You can adjust:

* **Minimum / Maximum nFeature_RNA**
* **Minimum / Maximum nCount_RNA**
* **Maximum mitochondrial percentage**

After modifying any threshold, click **[Run]** again to re-run the pipeline with the new thresholds. The raw data is retained, so this re-execution is fast and does not require re-uploading.

### 4. Restoring a Saved Session (.qs2)

Instead of an `.h5` file, you can also upload a previously saved `.qs2` session file. This instantly restores the full analysis state—including the processed Seurat object, repertoire annotations, and prior parameter settings—**without re-running the pipeline**.

This is the recommended way to:

* Resume previous work
* Share interim results with collaborators
* Skip the long preprocessing step when re-opening the app
...
#### 3. Lasso Selection and Recursive Sub-clustering

In addition to "Search & Highlight", SingleRepExplorer provides a **JavaScript-based Lasso selection** tool that lets you draw a freehand polygon directly on the UMAP/t-SNE plot to select an arbitrary cell population.

1.  **Enable Lasso Mode**: Click the **"Lasso Select"** button above the plot. The cursor switches to drawing mode.
2.  **Draw a Polygon**: Click-and-drag on the plot to outline the cells you wish to select. Release the mouse to close the polygon.
3.  **Assign a Label**: A dialog will appear asking you to provide a name for the selected subset (e.g., `Clonotype1_region`). The label is added as a new metadata column on every selected cell.
4.  **(Optional) Re-cluster the Subset**: After labeling, the application offers a **"Subset & Re-cluster"** action that:
    *   Extracts the selected cells into a new working object,
    *   Re-runs PCA / UMAP / clustering on this subset,
    *   Switches the Dimensional Plot to display the sub-clustered subset.
5.  **Persistent State**: The full-dataset analysis remains available — you can switch back at any time. Selected subsets persist across all other modules (DEG, Repertoire analyses, CellChat, etc.), so you can perform downstream analyses on the freshly defined population without re-uploading data.

This recursive, hypothesis-driven sub-analysis loop—**select → label → re-cluster → analyze**—is one of the platform's key usability innovations and replaces what would normally require multiple coding iterations.
...
* **Download plot (.pptx)**: Click to download the current plot as a high-resolution, publication-ready editable PowerPoint slide.
...
* **Download Table (.xlsx)**: Click to download this full data table as an Excel file.
...
*   **Download plot (.pptx)**: Download the current plot.
...
This module performs trajectory inference using **Slingshot** to model the biological processes (e.g., differentiation or activation) as a path through the single-cell expression space.

#### Sidebar Panel: Controls
1. **Target Subsets**: Select clusters to include in the trajectory analysis.
2. **Start Cluster**: Optionally define which cluster represents the "root" of the trajectory.
3. **Dimensionality Reduction**: Choose UMAP or TSNE for visualization.

#### Main Panel
* **Trajectory Plot**: UMAP/TSNE plot with the inferred lineage curve overlaid.
* **Pseudotime Download**: Export the calculated pseudotime values for each cell as an Excel (.xlsx) file.

### Phenotype-Repertoire Correlation

This module explores the relationship between gene expression (phenotype) and clonal expansion levels.

#### Sidebar Panel: Controls
1. **Repertoire Data**: Select TCR or BCR.
2. **Expansion Category**: Set thresholds for "Expanded" vs "Unexpanded" clones.
3. **Comparison**: Performs DGE between expanded and unexpanded cells.

### Cell-Cell Communication (CellChat)

Infers ligand–receptor signaling networks between cell clusters using **CellChat v2**. Two analysis modes are supported.

#### Sidebar Panel: Controls

1.  **Analysis Mode**:
    *   **Single Condition**: Run CellChat on the entire dataset.
    *   **Compare Two Conditions**: Run CellChat independently on each of two user-selected conditions and merge the results via `liftCellChat` and `mergeCellChat` for direct comparison.
2.  **Cell Annotation**: Choose the metadata column used as cell labels (e.g., `seurat_clusters`). To avoid CellChat's restriction on labels containing the digit `0`, the application automatically prefixes numeric labels with `Cluster ` (e.g., `Cluster 0`, `Cluster 7`).
3.  **Condition Column / Condition A / Condition B** (Compare mode only): Select the metadata column and the two condition values to compare (e.g., `sample = HCW_baseline` vs. `HCW_convalescent`).
4.  **Signaling Database**: Select the CellChatDB subset (`Secreted Signaling`, `ECM-Receptor`, `Cell-Cell Contact`, or `All`).
5.  **Plot Type**: Switches the main panel output:
    *   **Single Condition mode**: Circle plot, signaling strength heatmap, ligand-receptor bubble plot.
    *   **Compare mode**: `Pathway Strength Comparison` (rankNet), `Differential Interaction Network`, `Circle Plot (per condition)`, `Bubble Plot (per condition)` with selectable source/target clusters.
6.  **Run CellChat Inference**: This button starts the analysis (typically 5–10 minutes). Down-sampling is applied automatically (10,000 cells per single condition / 7,500 cells per condition in compare mode) to keep memory usage manageable.

#### Main Panel: Outputs

* Plots (circle / heatmap / bubble / rankNet) corresponding to the selected `Plot Type`.
* `Download plot (.pptx)`: Editable PowerPoint vector graphic.

*Note: CellChat is included in the official Docker image (v1.0.0+).*

### Multimodal Integration (CoNGA)

Identifies cells whose neighborhoods are similar in **both** transcriptomic and TCR/BCR sequence space, using a CoNGA-inspired (Clonotype Neighbor Graph Analysis) approach implemented entirely in R.

* The TCR/BCR sequence k-NN graph is built using **Levenshtein distance** between paired CDR3 sequences (`stringdist`).
* The transcriptomic k-NN graph is built from **PCA embeddings** (`RANN::nn2`).
* The CoNGA score for each cell is the **number of shared neighbors** across the two graphs.

#### Sidebar Panel: Controls

1.  **VDJ Type**: TCR or BCR.
2.  **K (Number of Neighbors)**: Size of the k-NN graphs (default 15).
3.  **Score Threshold**: Cells with CoNGA score above this value are highlighted in the right panel as "receptor-driven" cells.
4.  **Run CoNGA Analysis**: Starts the computation.

#### Main Panel: Outputs

A **dual-panel UMAP** visualization:

* **Left panel — `CoNGA Score Projection`**: Continuous color gradient of the CoNGA score across all cells.
* **Right panel — `Receptor-Driven Cells (Score ≥ N)`**: Cells exceeding the user-defined threshold are colored by Seurat cluster identity; remaining cells appear grey, making it easy to identify which clusters contain coordinated sequence–phenotype similarity.

`Download plot (.pptx)` exports the combined dual-panel figure as an editable PowerPoint slide.

*Note: This module's sensitivity is highest when applied to homogeneous immune compartments (e.g., sorted CD8+ T cells). Applied to heterogeneous PBMC mixtures, scores are typically broadly low because PCA embeddings are dominated by inter-lineage variance.*

### Automated Cell Type Annotation (LLM)

Automatically annotate clusters using Large Language Models (OpenAI GPT-4o) based on top marker genes.

#### Sidebar Panel: Controls
1. **API Key**: Required OpenAI API key.
2. **Biological Context**: Specify **Species**, **Tissue**, and **Granularity** to improve accuracy.
3. **Target Cluster Column**: Select the metadata column to annotate.

#### Main Panel
* **Annotation Results**: Table showing predicted cell types and biological reasoning.
* **Apply to Seurat**: Adds the predictions as a new metadata column in your object.
...
* **Download table (.xlsx)**:
    Click this button to download the complete data table (respecting the "TCR" or "BCR" selection) as an Excel (.xlsx) file for further analysis.
...
* **Plot**:
    ...
    * `Download plot (.pptx)`: Click to download the visualization as a publication-ready editable PowerPoint slide.
* **Table**:
    Shows the summary data table used to generate the plot, with exact counts and percentages.
    * `Download table (.xlsx)`: Click to download this summary data as an Excel (.xlsx) file.
...
* **Plot**:
    ...
    * `Download plot (.pptx)`: Downloads the bar chart as a publication-ready editable PowerPoint slide.
...
* **Table**:
    ...
    * `Download table (.xlsx)`: Downloads this summary table as an Excel (.xlsx) file.
...
* **Plot**:
    ...
    * `Download plot (.pptx)`: Downloads the rank-abundance plot as a publication-ready editable PowerPoint slide.
* **Table**:
    ...
    * `Download table (.xlsx)`: Downloads this summary data as an Excel (.xlsx) file.
...
* **Clonotype Tracking Plot**:
    ...
    * `Download plot (.pptx)`: Downloads the visualization as a publication-ready editable PowerPoint slide.
...
* **Plot**:
    ...
    * `Download plot (.pptx)`: Click to download the heatmap as a publication-ready editable PowerPoint slide.
* **Table**:
...
Analyzes the physicochemical properties of CDR3 amino acid sequences, such as hydrophobicity, charge, and molecular weight.

### Sequence Similarity

Calculates the sequence similarity (Levenshtein distance) between clonotypes to identify clusters of related sequences that share antigen specificity.

### BCR Lineage Tree

Constructs phylogenetic lineage trees for BCR clones to visualize somatic hypermutation and clonal evolution.

### BCR SHM & Isotype

Provides a combined view of BCR somatic hypermutation (SHM) rates and isotype usage across different cell clusters or samples.

*Note: This module is **temporarily hidden** in v1.0.0 due to column-structure issues in the underlying `combineBCR` output. The implementation remains in the source code (`app/module/4_14_shmIsotype.R`) and will be re-enabled in a future release once the column naming is stabilized.*