...
* **Pseudotime Download**: Export the calculated pseudotime values for each cell as a CSV.
...
Infers signaling networks between cell clusters and compares networks across clonal expansion states using **CellChat**.

*Note: This feature is included in the official Docker image (v1.0.0+).*

### Multimodal Integration (CoNGA)

Integrated analysis of TCR/BCR sequences and gene expression using the **CoNGA** (Clonotype Neighbor Graph Analysis) framework. Identify clusters based on both transcriptomic and repertoire similarity.
