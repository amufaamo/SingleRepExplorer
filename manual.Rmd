...
かしこまりました！`reductionPlot` モジュールのコードを拝見しました。
クラスタリングの可視化だけでなく、**グループ名の変更機能**や、TCR/BCR情報と連携した**強力な検索＆ハイライト機能**まで搭載されていて、本当に素晴らしいですね！

この機能に基づいた、英語の解説書を作成します。

---

### 3.2. [Dimensional Plot] Tab
...
Here is the documentation for the Differential Gene Expression module.

---

### 3.3.2. [Transcriptome] - Differential Expression
...
* **Differentially Expressed Features...**:
    ...
    * `Download all table (.csv)`: Downloads this data table.
    ...
    Here is the documentation for the Clonotype Information module.

---

### 3.4.1. [Repertoire] - Clonotype Information
...
Here is the documentation for the Unique Clones module.

---

### 3.4.3. [Repertoire] - Unique Clones
...
わぁ、これは`Clonotype Frequency`（クロノタイプ頻度）と`Gene Usage`（遺伝子使用頻度）の**両方を分析できる、とても強力なモジュールですね！**
UIとサーバーのコード、拝見しました。`analysis_target` で処理を切り替える設計、素晴らしいです！

このモジュールのための解説書（英語）を作成しますね。

---

### 3.4.2. [Repertoire] - Frequency Analysis
...
Here is the documentation for the Clonal Length module.

---

### 3.4.5. [Repertoire] - Clonal Length
...
Here is the documentation for the Track Clonotype module.

---

### 3.4.7. [Repertoire] - Track Clonotype
...
* **VDJ Type**: Select whether to analyze **TCR** or **BCR** data.
* **クローン識別子列 (Clone Identifier Column)**: Choose which column to use for identifying clonotypes. This is a crucial setting.
    * e.g., `raw_clonotype_id`, `TCR_pair_CTaa` (for paired CDR3 amino acids), or `BCR_IGH_cdr3`.
* **追跡対象クローン (Target Clonotypes)**: Based on your column selection, this box allows you to select one or *multiple* clonotypes from the list that you wish to track.
* **Group by**: Choose the metadata column that defines your groups for tracking (e.g., `sample`, `seurat_clusters`).
* **プロットタイプ (Plot Type)**: Select the visualization style:
    * `積み上げ棒グラフ (Stacked Bar)`: Shows a bar for each group, where each segment's height represents the abundance of a tracked clonotype.
...
* **表示値 (Value Type)**: Toggles the Y-axis.
    * `頻度 (%) (frequency)`: Shows the clonotype's abundance as a percentage *within that group's total cells*.
    * `細胞数/リード数 (count)`: Shows the raw cell count for that clonotype in that group.
* **グループ順序 (Group Order)**: Enter a comma-separated list to define the exact order of the groups on the X-axis (e.g., `Sample_A, Sample_B, Sample_C`). This is essential for Alluvial plots.
...
* **クローン追跡プロット (Clonotype Tracking Plot)**:
...
* **追跡結果テーブル (Tracking Results Table)**:
...
Here is the documentation for the Clonal Overlap module.

---

### 3.4.8. [Repertoire] - Clonal Overlap
...
* **VDJ Type**: Select whether to analyze **TCR** or **BCR** data.
* **クローン識別子列 (Clone Identifier Column)**: This is a key setting. You must define what constitutes a "clonotype" for the overlap calculation.
...
はい、承知いたしました！`clonalProportion` モジュールのコードを拝見しました。クローンサイズ（同じクローンが何回出現したか）に基づいて、各グループがどのようなクローンサイズ分布を持っているかを可視化する機能ですね！✨

このモジュールのための解説書（英語）を作成します。

---

### 3.4.9. [Repertoire] - Clonal Proportion
...
* **クローン定義方法 (Clone Definition)**: Select which column defines a unique clonotype.
...
* **グループ別クローンサイズ割合 (Clonal Proportion by Size Category Plot)**:
...
* **割合データテーブル (Proportion Data Table)**:
...
Here is the documentation for the Antigen Prediction module.

---

### 3.4.10. [Repertoire] - Antigen Prediction
...
* **テーブルダウンロード (Download table (.csv))**: Click this button to download the complete **source antigen database file** (TCR or BCR) that is being used for the comparison.

### Main Panel: Outputs

* **抗原予測結果テーブル (Antigen Prediction Table - Matched CDR3s)**:
...
はい、承知いたしました！`phylogeneticTree` モジュールのコードを拝見しました。

**注意点:** 現在のコードを見ると、`dowser` を使った系統樹の構築 (`getTrees`) や `ggtree` を使ったプロット (`ggtree`, `geom_tippoint` など) の部分はコメントアウトされているか、実装されていないようです。現在の機能としては、**特定のBCRクローン（複数のサブクローンを持つもの）を選択し、そのクローンに属する細胞の生データをテーブルで表示する**ことが中心のようですね。

この現在の機能に基づいた解説書（英語）を作成します。将来的に系統樹の描画機能が実装された際に、解説を追記できます。

---

### 3.4.11. [Repertoire] - Phylogenetic Tree (Data Viewer)
...
はい、承知いたしました！`clonalAbundance` モジュールのコードを拝見しました。クローンの存在量（Abundance）をランク順にプロットして、グループ間の分布の違いを比較する機能ですね！📈

このモジュールのための解説書（英語）を作成します。

---

### 3.4.4. [Repertoire] - Clonal Abundance
...
はい、承知いたしました！`publicClonotype` モジュールのコードを拝見しました。複数のグループ間で共通して存在するクローン（Public Clonotype）を特定し、ベン図で可視化する機能ですね！選択した共通部分の詳細を確認できるのも便利です！📊

このモジュールのための解説書（英語）を作成します。

---

### 3.4.9. [Repertoire] - Public Clonotype
